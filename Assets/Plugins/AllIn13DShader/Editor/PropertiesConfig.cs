...
namespace AllIn13DShader
{
	...
	public class PropertiesConfig
	{
				...
				else
				{
					if (attributes.Length > 0)
					{
						...
						else if (firstAttribute.StartsWith(Constants.EFFECT_PROPERTY_ATTRIBUTE_PREFIX))
						{
							...
							ConfigureProperty(effectPropertyAttributeData, i, propertyName, displayName, shaderPropertyFlags);
						}
		...
		private void ConfigureProperty(EffectPropertyAttributeData data, int propertyIndex, string propertyName, string displayName, ShaderPropertyFlags shaderPropertyFlags)
		{
			AllIn13DEffectConfig effectConfig = FindEffectConfigByID(data.parentEffectID);
			EffectProperty effectProperty = effectConfig.CreateEffectProperty(propertyIndex, propertyName, displayName, shaderPropertyFlags, data);
		}
